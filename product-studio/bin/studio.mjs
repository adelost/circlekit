...
export async function main(args = process.argv.slice(2), { cwd = process.cwd(), stdout = process.stdout } = {}) {
  ...
  try {
    ...
    if(args[0]==='live'&&args[1]==='connect') {
      ...
      if(args.includes('--help')){stdout.write('v1d-studio live connect --device SERIAL [--product ID] [--port 17317]\nReads the selected project nativeLive package/action, stages one private ticket and never installs or replaces an adb mapping.\n');return 0;}
