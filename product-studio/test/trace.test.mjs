...
test('raw payloads and future causal parents refuse',()=>{const r=recorder();assert.throws(()=>r.record({...event(),payload:{secret:'not allowed'}}),/Unexpected/);assert.throws(()=>r.record({...event(),causedBy:200}),/earlier/);});
