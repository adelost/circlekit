 ...
 */
@Immutable
sealed interface CircleContentScale {
    @Immutable
    data object FixedAtoms : CircleContentScale

    @Immutable
    data class CanonicalFit(val canonicalSideDp: Float) : CircleContentScale {
        init {
...
enum class CircleViewportBoundaryEdge { BOTTOM, END }

/** JSON-like layout data; renderers consume it without inferring a form factor. */
@Immutable
data class CircleComponentViewportSpec(
    val sideDp: Float,
    val contentScale: CircleContentScale,
    val boundaryEdge: CircleViewportBoundaryEdge? = null,
) {
    init {
 ...
 */
@Composable
fun CircleComponentViewport(
    spec: CircleComponentViewportSpec,
    ...
    Box(modifier = modifier.size(spec.sideDp.dp)) {
        ...
        if (boundaryVisible) {
            when (spec.boundaryEdge) {
                CircleViewportBoundaryEdge.BOTTOM -> Box(
                    Modifier
                ...
                CircleViewportBoundaryEdge.END -> Box(
                    Modifier
