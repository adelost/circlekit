...
function emitSwift(
  ...
  const serviceIds = product.services.map(({ id }) => id);
  const portIds = unique([
    ...product.portRegistry.servicePorts,
  ...
  requireDistinctSwiftCases(serviceIds, "native service");
...
${serviceIds.map((id) => `    case ${swiftCase(id)} = ${swiftString(id)}`).join("\n")}
}
...
${product.services.map((service) => `        ShowcaseService(id: .${swiftCase(service.id)}, typeRef: ${swiftString(service.serviceTypeRef)})`).join(",\n")}
    ]
