...
function logicView() {
  ...
  if (!['machine','decision-table'].includes(f.kind)) return banner(f.blockedReason ?? 'No specialized runner is available.') + panel('Exported facet', `<pre class="panel-body">${escape(pretty(f.compiled))}</pre>`);
  if (f.kind === 'machine') {
    const m = f.compiled;
