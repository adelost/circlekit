...
${scenarios}
  ...
  const services = product.services.map((service) =>
    `        ShowcaseServiceDescriptor(${kotlinString(service.id)}, ${kotlinString(service.serviceTypeRef)}),`).join("\n");
  const ports = [...product.portRegistry.servicePorts, ...product.portRegistry.componentPorts].map((item) =>
    `        ShowcaseProductPort(${[
