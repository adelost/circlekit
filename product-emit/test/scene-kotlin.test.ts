...
const scenes: readonly CompiledScene[] = [
  {
    ...
    layers: [
      ...
      { id: "map-base-copy", renderer: "tiles", style: "raster", source: { kind: "fetch", id: "MAP_TILES" } },
    ],
  },
  {
...
test("a generic scene emitter generates the ordered declared values against the shared scene ABI", () => {
  ...
  const firstLayer = kotlin.indexOf('GeneratedSceneLayer(id = "map-base"');
  const pointLayer = kotlin.indexOf('GeneratedSceneLayer(id = "map-points"');
  const repeatedLayer = kotlin.indexOf('GeneratedSceneLayer(id = "map-base-copy"');
  const replayLayer = kotlin.indexOf('GeneratedSceneLayer(id = "recorded-path"');
