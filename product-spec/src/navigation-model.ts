...
export interface NavigationRouteIntentContract extends LegoContract {
  readonly kind: "event";
...
export interface ProductNavigationDeclaration<PageId extends string = string> {
  ...
  readonly routeIntentContract: NavigationRouteIntentContract;
}

export interface CompiledArtifactPage {
...
export interface CompiledNavigationArtifact {
  readonly artifactRef: string;
  readonly entryPageRef: string;
  readonly pages: readonly CompiledArtifactPage[];
}

export interface CompiledProductAction {
  readonly id: string;
...
export interface CompiledProductActionGroup {
  readonly componentInstanceRef: string;
  readonly pageRefs: readonly string[];
  readonly artifactRefs: readonly string[];
  readonly actions: readonly CompiledProductAction[];
}
...
export interface ProductNavigationIr {
  ...
  readonly artifacts: readonly CompiledNavigationArtifact[];
  readonly activePageContract: NavigationActivePageContract;
  readonly activePagePortRef: string;
  readonly pageHostPortRef: string;
  readonly routeIntentContract: NavigationRouteIntentContract;
  readonly actionGroups: readonly CompiledProductActionGroup[];
}

/** One route event for the whole navigation: its target lives only in the closed payload. */
export function navigationRouteContract(id: string): NavigationRouteIntentContract {
  const contract = {
    id: `${id}.route-intent`, kind: "event", boundary: "ui-event",
    fields: [field("target", finiteValueRef(`${id}.page`))],
    navigation: { kind: "route", effect: "push" },
  } as const;
...
export function compileProductNavigation(input: {
  ...
  const compiledArtifacts = artifacts.map((artifact): CompiledNavigationArtifact => {
    ...
    const pages = artifact.screenRefs.map((pageRef): CompiledArtifactPage => {
      const page = pageById.get(pageRef);
      if (page === undefined) throw new Error(`artifact '${artifact.id}' selects undeclared page '${pageRef}'`);
      ...
      return {
        pageRef,
        restore: pageRef === artifact.entryScreen ? "root" : "process",
        guardContractRef: page.guard?.id ?? null,
        back: page.back,
      };
    });
    return { artifactRef: artifact.id, entryPageRef: artifact.entryScreen, pages };
  });
  ...
  if (!sameNavigationContract(contractById.get(declaration.activePageContract.id), declaration.activePageContract)) {
    throw new Error(`navigation active page output contract drift`);
  }
  const activeOutput = activeOutputs[0]!;
  const pageHostBindings = graph.portRegistry.bindings.filter((binding) =>
    binding.kind === "component-input" && binding.from === activeOutput.ref);
  if (pageHostBindings.length !== 1) {
    throw new Error(`navigation active page must bind exactly one page-host component input (found ${pageHostBindings.length})`);
  }
  const pageHostPortRef = pageHostBindings[0]!.to;
  const pageHostPort = graph.portRegistry.componentPorts.find(({ ref }) => ref === pageHostPortRef);
  if (pageHostPort?.contractRef !== declaration.activePageContract.id) {
    throw new Error(`navigation page-host '${pageHostPortRef}' contract drift`);
  }
  const pageHostId = pageHostPort.ownerId;
  for (const artifact of artifacts) {
    for (const pageRef of artifact.screenRefs) {
      const mounted = artifactScopes.some((scope) => scope.artifactRef === artifact.id
        && scope.screenRef === pageRef
        && scope.includedMounts.some(({ componentInstanceRef }) => componentInstanceRef === pageHostId));
      if (!mounted) throw new Error(`page-host '${pageHostId}' is not mounted on '${artifact.id}/${pageRef}'`);
    }
  }
  ...
  const actionGroups = [...new Set(actionOutputs.map(({ ownerId }) => ownerId))].sort()
    .map((ownerId): CompiledProductActionGroup => {
      ...
      const actions = actionOutputs.filter((port) => port.ownerId === ownerId).map((source): CompiledProductAction => {
        ...
        if (isRouteContract(contract)) {
          if (!sameNavigationContract(contract, declaration.routeIntentContract)) {
            throw new Error(`navigation action '${source.ref}' route-intent contract drift`);
          }
          const target = graph.portRegistry.nodePorts.find(({ ref }) => ref === binding.to);
          if (target?.ownerId !== activeOutput.ownerId) {
          ...
          return {
            ...
            targetPortRef: binding.to, contractRef: source.contractRef, effect: "push",
          };
        }
        return {
          id: source.portId, kind: "event", sourcePortRef: source.ref,
          targetPortRef: binding.to, contractRef: source.contractRef, effect: "dispatch",
        };
      });
  ...
  return {
    ...
    pages: declaration.pages.map((page) => ({
      id: page.id, guardContractRef: page.guard?.id ?? null, back: page.back,
    })),
    artifacts: compiledArtifacts,
    activePageContract: declaration.activePageContract,
    activePagePortRef: activeOutput.ref,
    pageHostPortRef,
    routeIntentContract: declaration.routeIntentContract,
    actionGroups,
  };
}
...
function validateDefinition(definition: ProductNavigationDeclaration): void {
  ...
  if (!sameNavigationContract(definition.activePageContract, navigationActivePageContract(definition.id))) {
    throw new Error(`navigation '${definition.id}' active page contract drift`);
  }
  if (!sameNavigationContract(definition.routeIntentContract, navigationRouteContract(definition.id))) {
    throw new Error(`navigation '${definition.id}' route-intent contract drift`);
  }
}
