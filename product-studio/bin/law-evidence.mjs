...
function parse(args) {
  ...
  for(let index=0;index<args.length;index++) {
    ...
    requireThat(['--root','--product','--output','--repository'].includes(key),'cli.usage','Unknown law-evidence option '+key+'.');
  ...
  requireThat(options['--root']&&options['--output'],'cli.usage',
    'Use --root ROOT [--product ID] --output test-results/declaration-laws.json.');
  return options;
}
