...
class CircleUiProfileTest {
    @Test
    fun `canonical Watch profile is independent of host resolution`() {
        val profile = CircleUiProfiles.WatchCanonical

        assertEquals(CircleSurfaceClass.ROUND, profile.surfaceClass)
    ...
    fun `phone layout changes capacity without deriving atom scale`() {
        val compact = CircleUiProfiles.PhoneCompact
        val wide = CircleUiProfiles.PhoneWide
    ...
    fun `phone profiles cannot claim the round watch surface`() {
        assertThrows(IllegalArgumentException::class.java) {
            CircleUiProfiles.phoneProfileFor(CircleSurfaceClass.ROUND)
        }
    }
}
