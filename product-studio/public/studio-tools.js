...
export function traceView(project, state, escape) {
  ...
  return `<section class="panel"><header class="panel-head"><h2>${heading} <code>${escape(trace.sessionId)}</code></h2><div class="toolbar"><button data-action="import-trace" class="quiet-link">Import another</button>${trace.version===2&&state.liveStatus?.sessions?.some(session=>session.id===trace.capture.id&&session.ending!=='frozen')?'<button data-action="freeze-trace" class="quiet-link">Freeze cut</button>':''}<button data-action="export-trace" class="quiet-link">Export trace</button></div></header>
    <div class="panel-body">${liveStatusLine(trace,state,escape)}<details class="exact-key"><summary>${trace.eventCount} events · ${dropped(trace)} dropped${trace.truncation.gaps.length ? ` · ${trace.truncation.gaps.length} gaps` : ''}</summary><small>${escape(trace.notice)}</small></details>
      ...
      <details class="view-menu"><summary>Filter</summary><div class="fact-grid"><label>Filter recorded text<input id="trace-search" value="${escape(state.traceSearch ?? '')}" placeholder="Entity, cell or summary"></label><label>Operation ID<input id="trace-operation" value="${escape(state.traceOperation ?? '')}" placeholder="Exact operation ID"></label><button data-action="trace-filter">Apply</button></div></details>
      ${frame?.current ? `<div class="notice info"><strong>#${frame.current.sequence} · ${escape(frame.current.phase??'legacy phase unspecified')} ${escape(frame.current.kind)} · ${traceTime(frame.current.atMs,trace.clock.domain)}</strong><p>${escape(frame.current.summary ?? '')}</p><button data-entity="${escape(frame.current.entityKey)}">Inspect ${escape(entityLabel(project,frame.current.entityKey))}</button></div>` : ''}
