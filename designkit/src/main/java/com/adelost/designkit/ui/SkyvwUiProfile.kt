...
/**
 * One explicit sizing policy for every Skyvw renderer.
 *
 ...
 * one proportional viewport opt into [SkyvwContentScale.CanonicalFit] locally;
 * ordinary rows, buttons and type remain fixed atoms.
 */
@Immutable
data class SkyvwUiProfile(
...
object SkyvwUiProfiles {
    ...
    val WatchCanonical = SkyvwUiProfile(
     ...
     * app profile (skyvw:4 P1-7: no caller may invent a third profile).
     */
    val PhoneCompact = SkyvwUiProfile(
        surfaceClass = SkyvwSurfaceClass.PHONE_COMPACT,
        atomScale = 1f,
        canonicalRoundViewportDp = null,
    )

    val PhoneWide = SkyvwUiProfile(
        surfaceClass = SkyvwSurfaceClass.PHONE_WIDE,
        atomScale = 1f,
        canonicalRoundViewportDp = null,
    )

    /** Resolve the named profile for a non-round surface class. */
    fun phoneProfileFor(surfaceClass: SkyvwSurfaceClass): SkyvwUiProfile = when (surfaceClass) {
        SkyvwSurfaceClass.PHONE_COMPACT -> PhoneCompact
        SkyvwSurfaceClass.PHONE_WIDE -> PhoneWide
        SkyvwSurfaceClass.ROUND ->
            throw IllegalArgumentException("round surfaces use WatchCanonical, never a phone profile")
    }
}

val LocalSkyvwUiProfile = staticCompositionLocalOf { SkyvwUiProfiles.WatchCanonical }

@Composable
@ReadOnlyComposable
fun skyvwUiProfile(): SkyvwUiProfile = LocalSkyvwUiProfile.current
