...
For test evidence, `v1d-studio record --product my-product` runs that workspace's explicit `recordCommand` argv only when requested. `-- TEST_COMMAND` remains an explicit override. It derives the trace identity and `test-results/<project-id>-studio-trace.json` from the workspace. ProductSpec's test-only package condition observes JS decisions and machine steps; generated Kotlin looks only when its build's `DEBUG` constant is true. Named port implementations can use `bindPortImplementations`, which returns the same object in normal builds. Recording refuses an empty or failed test run and preserves the previous trace.
