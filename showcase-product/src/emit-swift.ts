...
function emitSwift(
  ...
  const caseComponentIds = componentIds.filter((id) =>
    product.showcase.cases.some((candidate) => candidate.id === id));
  const components = caseComponentIds.map((id) => {
  ...
  const componentRegistrations: readonly NativeComponentRendererRegistration[] = componentIds.map((componentId) => {
    ...
    const eventEmitter = componentId === "page.menu"
      ? defineNativeTypedEventEmitter([
        nativeEvent("page.menu.route", "navigation.route", "showcase.navigation.route-intent"),
        ...product.showcase.cases.map(({ openPort }) =>
          nativeEvent(`page.menu.${openPort}`, `navigation.${openPort}`, "showcase.open-action")),
      ])
  ...
  const nativeNodes = compileShowcaseNativeNodes(product, profiles, [
    ...
    { nodeId: "renderer", nativePortId: "ShowcaseNativeEnvironment" },
    { nodeId: "renderer.presentation", nativePortId: "ShowcaseNativeEnvironment" },
  ]);
...
${componentBindings.map(({ component, ref }) => `        ShowcaseComponent(
            ...
            openPort: ${swiftString(component.openPort)},
...
${trees.map(({ artifact, scope, tree }) => `        ShowcaseTree(
            ...
            mounts: [
${scope.includedMounts.filter(({ componentInstanceRef }) => caseComponentIds.includes(componentInstanceRef)).map((included) => {
...
${componentRegistrations.map((registration) => `        ShowcaseNativeComponentRegistration(
            ...
            mounts: [${registration.mounts.map((mount) => `ShowcaseNativeMountRegistration(
                profileId: .${swiftCase(mount.profileRef)},
            ...
            immutableInputs: [${registration.immutableInputs.map((input) => `ShowcaseNativeInputRegistration(
                ...
                producerPortRef: ${swiftString(input.producerPortRef)},
                contractRef: ${swiftString(input.contractRef)},
                required: ${input.required},
                read: { environment in environment.read(producerPortRef: ${swiftString(input.producerPortRef)}) }
            )`).join(", ")}],
            eventEmitter: ${registration.eventEmitter.kind === "empty"
    ? `.empty({ _ in fatalError("read-only Showcase renderer cannot emit") })`
    : `.typed([${registration.eventEmitter.bindings.map((binding) => `ShowcaseNativeEventBindingRegistration(
...
${nativeNavigation.artifacts.map((artifact) => `        ShowcaseNavigationArtifact(
            artifactId: .${swiftCase(artifact.artifactRef)},
            entryPageRef: ${swiftString(artifact.entryPageRef)},
            pages: [${artifact.pages.map((page) => `ShowcaseNavigationPage(pageRef: ${swiftString(page.pageRef)}, restore: ${swiftString(page.restore)}, back: ${swiftString(page.back)}, guardContractRef: ${page.guardContractRef === null ? "nil" : swiftString(page.guardContractRef)})`).join(", ")}]
        )`).join(",\n")}
    ]
    static let activePageBindings: [ShowcaseActivePageBinding] = [
${nativeNavigation.activePageBindings.map((binding) => `        ShowcaseActivePageBinding(publisherPortRef: .${swiftCase(binding.publisherPortRef)}, pageHostPortRef: .${swiftCase(binding.pageHostPortRef)})`).join(",\n")}
    ]
    static let navigationActionGroups: [ShowcaseNavigationActionGroup] = [
${nativeNavigation.actionGroups.map((group) => `        ShowcaseNavigationActionGroup(
            artifactId: .${swiftCase(group.artifactRef)},
            componentId: .${swiftCase(group.componentInstanceRef)},
            actions: [${group.actions.map((action) => `ShowcaseNavigationAction(sourcePortRef: .${swiftCase(action.sourcePortRef)}, targetPortRef: .${swiftCase(action.targetPortRef)}, effect: ${swiftString(action.effect)})`).join(", ")}]
        )`).join(",\n")}
    ]
}
`;
